...
bool MpptRS485::updateRead(){
  ...
  // Overall read timeout: 35ms (was 50ms)
  if (now - readT0 >= 35) {
    processRead();
    reading = false;
    return true;
  ...
  // Inter-byte gap timeout: 5ms (was 10ms)
  if(readN > 0 && now - readLastByte >= 5) {
    processRead();
    reading = false;
    return true;
  }
...
bool MpptRS485::processRead(){
  ...
  for(int e = 24; e < readN; e++){
    ...
    if (!readIsSettings) {
      ...
      if (readCommand == 3) {
        // Command 3 (instantaneous): byte 15 = inst_duty_cycle, byte 16 = pwm_min, byte 17 = mppt_status, byte 18 = heartbeat
        mpptData.inst_duty_cycle = f[15];
        mpptData.pwm_min = f[16];
        mpptData.mppt_status = f[17];
        mpptData.heartbeat = f[18];
        Serial.print("  MPPT Status byte: ");
        Serial.println(mpptData.mppt_status);
        Serial.print("  Inst Duty Cycle: ");
        Serial.print(mpptData.inst_duty_cycle);
        Serial.println("%");
        Serial.print("  PWM Min: ");
        Serial.println(mpptData.pwm_min);
        Serial.print("  Heartbeat: ");
        Serial.println(mpptData.heartbeat);
      } else {
        // Command 9 (average/home): bytes 15-16 = vset, byte 17 = duty_cycle
        mpptData.vset = u16be(f[15], f[16]);
      ...
      if (readCommand == 3) {
        Serial.print(" Status=");
        Serial.print(mpptData.mppt_status);
        Serial.print(" InstDC=");
        Serial.print(mpptData.inst_duty_cycle);
        Serial.print("% PWMmin=");
        Serial.print(mpptData.pwm_min);
        Serial.print(" HB=");
        Serial.print(mpptData.heartbeat);
      } else {
        Serial.print(" Vset=");
        Serial.print(mpptData.vset);
        Serial.print(" Duty=");
