...
bool OledUI::begin() {
  Wire.begin();
  Wire.setClock(400000UL);   // 400kHz Fast-mode I2C (was 100kHz)
...
void OledUI::drawInstantaneousMpptCard(const MpptData& d) {
  ...
  if (d.valid) {
    // If ADC returns junk (e.g. 0xFFFE when no current), treat as zero
    uint16_t ib_raw = (d.ib > 1023) ? 0 : d.ib;
    
    // Apply scaling formula: raw ADC value * 0.1212121212 (no midpoint calibration)
    uint32_t current_centi = ((uint32_t)ib_raw * 12121212UL + 500000UL) / 1000000UL;
    
    uint16_t ib_i = (uint16_t)(current_centi / 100);
    uint8_t  ib_d = (uint8_t)(current_centi % 100);

    // Power = Vb * current_centi / 100
    uint32_t power = ((uint32_t)d.vb * current_centi + 50) / 100;
    ...
    switch (d.mppt_status) {
      ...
      case 4: status_char = 'A'; break;
      case 5: status_char = 'P'; break;
      case 6: status_char = 'B'; break;
    ...
    snprintf(s, sizeof(s), "PV:%u V", d.vp);
    _u8.drawStr(0, 26, s);
    snprintf(s, sizeof(s), "C:%u%c(%u)", d.temp_coil, 0xB0, d.heartbeat);
    _u8.drawStr(70, 26, s);
